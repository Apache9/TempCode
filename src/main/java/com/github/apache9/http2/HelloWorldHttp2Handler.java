...
public class HelloWorldHttp2Handler extends Http2ConnectionHandler {
    ...
    private static final Http2FrameLogger LOG = new Http2FrameLogger(INFO,
            HelloWorldHttp2Handler.class);

    static final ByteBuf RESPONSE_BYTES = unreleasableBuffer(copiedBuffer("Hello World",
            CharsetUtil.UTF_8));

    public HelloWorldHttp2Handler() {
        this(new DefaultHttp2Connection(true), new Http2InboundFrameLogger(
                new DefaultHttp2FrameReader(), LOG), new Http2OutboundFrameLogger(
                new DefaultHttp2FrameWriter(), LOG), new SimpleHttp2FrameListener());
    }
    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
        if (evt instanceof HttpServerUpgradeHandler.UpgradeEvent) {
            ...
            Http2Headers headers = new DefaultHttp2Headers().status(OK.codeAsText()).set(
                    new AsciiString(UPGRADE_RESPONSE_HEADER), new AsciiString("true"));
